...
public interface StatementRepo extends JpaRepository<Statement, Long> {
	...
	@Modifying
	@Transactional
	@Query("update Statement s set s.dueAmount = :amount where s.billingDate= :billingDate And s.customer.id= :customerId")
	public void setAmountForStatementWhenTransactionUpdated(double amount,LocalDate billingDate,long customerId);
